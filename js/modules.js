const Modules = {
    definitions: {
        ...
        critCore: {
            ...
            unlockHint: () => '需要: 速度Lv.3 或 解锁自动收集器'
        },
        timeWarp: {
            id: 'timeWarp',
            name: '时间扭曲',
        ...
        quantumFold: {
            ...
            getOutput: (level) => {
                ...
                return { chance: Math.min(0.3, level * 0.03), mult: 1 + level * 0.2 };
            },
            canUnlock: () => Modules.getTotalLevels() >= 20,
            unlockHint: () => '需要: 模块总等级≥20'
        },
        voidMirror: {
            id: 'voidMirror',
            name: '虚空之镜',
            ...
            costMult: 2.5,
            maxLevel: 10,
            requires: ['quantumFold'],
            getOutput: (level) => {
                if (level <= 0) return 1;
                const tokenBonus = Math.log10(Math.max(10, Game.tokens)) * 0.01 * level;
                return 1 + tokenBonus;
            },
            canUnlock: () => Modules.isUnlocked('quantumFold') && Game.tokens >= 10000,
            unlockHint: () => '需要: 解锁量子折叠 + 10,000◆'
        }
    },
    unlocked: {},
    panelOpen: false,
    ...
    togglePanel() {
        ...
        // 修复: 展开时强制刷新
        if (this.panelOpen) this.renderModulePanel();
    },
    closePanel() {
    ...
    upgrade(moduleId) {
        ...
        Game.tokens -= cost;
        if (!this.unlocked[moduleId]) this.unlocked[moduleId] = { level: 0, unlockedAt: Date.now() };
        this.unlocked[moduleId].level++;
        if (typeof Recorder !== 'undefined') Recorder.recordState('moduleLevel', this.unlocked[moduleId].level);
        this.save();
        if (currentLevel === 0) {
            Cube.bigBounce();
            Cube.glow(0x00D4FF);
            UI.showFloatText(window.innerWidth / 2, window.innerHeight / 2, `解锁 ${def.name}!`, 'levelup');
        } else {
            Cube.bounce();
            Cube.glow(def.color.replace('#', '0x'));
            UI.showFloatText(window.innerWidth / 2, window.innerHeight / 2, `${def.name} Lv.${this.unlocked[moduleId].level}!`, 'gold');
        }
        // 修复: 购买后立即刷新面板
    ...
    getAutoOutput() {
        let baseOutput = 0, speedMult = 1, voidMult = 1;
        for (const [id, data] of Object.entries(this.unlocked)) {
            ...
            switch (id) {
                case 'autoCollector': baseOutput += output; break;
                case 'timeWarp': speedMult *= output; break;
                case 'voidMirror': voidMult *= output; break;
            }
        }
        baseOutput *= speedMult * voidMult;
    ...
    renderModulePanel() {
        ...
        for (const [id, def] of Object.entries(this.definitions)) {
            ...
            if (typeof output === 'number') {
                if (id === 'autoCollector') outputText = `+${output >= 100 ? output.toFixed(0) : output.toFixed(1)}/s`;
                else if (id === 'timeWarp') outputText = `×${output.toFixed(2)}`;
                else if (id === 'voidMirror') outputText = `×${output.toFixed(2)}`;
            ...
            let hintHtml = '';
            if (!isUnlocked && def.unlockHint) {
                hintHtml = `<div class="module-hint">${def.unlockHint()}</div>`;
            }
            card.innerHTML = `
                ...
                <div class="module-info">
                    ...
                    ${isUnlocked ? `<div class="module-output" style="color: ${def.color}">${outputText}</div>` : hintHtml}
                </div>
                <div class="module-level">
                    ${isUnlocked ? `
                        <span class="level-num">Lv.${level}</span>
                        <span class="upgrade-cost ${canAfford ? 'can-buy' : 'cant-buy'}">▲ ${cost}</span>
                    ` : canUnlock ? `
                        <span class="unlock-btn ${canAfford ? 'can-buy' : 'cant-buy'}">解锁 ${cost}◆</span>
                    ` : `
        ...
        totalDiv.innerHTML = `<span>总自动产出</span><span style="color: #FFD700; font-weight: bold;">+${totalOutput.toFixed(1)}/s</span>`;
